...
from train.utils import instantiate_from_config, dummy_context
...
class CategoricalDiffusionModel(nn.Module):
    def __init__(self, diffusion_model_spec, denoising_model_spec,
                 num_classes, num_timesteps, condition_channels, spatial_size, train_ddim_sigmas=False):
        super().__init__()
        ...
        self.unet = get_cls_from_pkg(denoising_model_spec["unet"],
                                     ...
                                     out_channels=num_classes, spatial_size=spatial_size)
        ...
        if self.train_ddim_sigmas:
            self.optimizer = torch.optim.AdamW([self.trainable_ddim_sigmas], lr=1e-2)
        
    def eval(self, eval_all=False):
        self.diffusion_model.eval()
        self.unet.eval()
        self.denoising_model.eval()
        if eval_all: self.eval()
    
    def forward(self, *args, **kwargs):
        return self.denoising_model.forward(*args, **kwargs)
    
    def register_buffers(self, ddim_discretize="uniform", ddim_steps=50, ddim_eta=1., verbose=False):
        ...
        to_torch = lambda x: x.clone().detach().to(torch.float32).to(self.alphas_cumprod.device)
        ...
        if self.train_ddim_sigmas: self.trainable_ddim_sigmas = nn.Parameter(self.ddim_sigmas, requires_grad=True)

    def ddim_sampling(self, x_noise, condition, feature_condition, context, verbose=False,
                      ddim_use_original_steps=False, mask=None, x0=None, timesteps=None, log_every_t=10,
                      ref_class=None, loss_fn=None):

        device = self.betas.device
        b = x_noise.shape[0]
        if self.train_ddim_sigmas: assert self.optimizer is not None and ref_class is not None
        ...
        for i, step in enumerate(iterator):
            ...
            outs = self._ddim_sampling(x_noise, condition, feature_condition, context, ts, self.unet.dims, 
                                       ...
                                       ref_class=ref_class, loss_fn=loss_fn)
            x_noise = outs["x_noise"]
            pred_x0 = outs["pred_x0"]
            loss = outs["loss"]
            if self.train_ddim_sigmas and loss is not None:
                cum_loss += loss.item()
        ...
        return x_noise, intermediates, cum_loss
        
    def _ddim_sampling(self, x, c, f, ctxt, t, d,
                       index, use_original_steps, temperature=1., noise_dropout=0.,
                       ref_class=None, loss_fn=None, sigma_t=0.):
        b, *_, device = *x.shape, x.device

        with torch.no_grad() if not self.train_ddim_sigmas else dummy_context():
            ot = self.denoising_model.forward_step(x, c, f, t, ctxt)
            et = self.diffusion_model.theta_post_prob(x, ot["diffusion_out"], t)
        lt = ot["cond_pred_logits"]
        loss = None
        if ref_class is not None: 
            loss = torch.clip(loss_fn(lt.softmax(1), ref_class), 0, 1) * (1 - index / len(self.ddim_timesteps))
        ...
        sigmas = self.ddim_sigmas_for_original_num_steps if use_original_steps else self.ddim_sigmas if not self.train_ddim_sigmas else self.trainable_ddim_sigmas
        # select parameters corresponding to the currently considered timestep
        a_t = torch.full((b, 1) + (1,) * d, alphas[index], device=device)
        a_prev = torch.full((b, 1) + (1,) * d, alphas_prev[index], device=device)
        sigma_t = sigmas[index].expand_as(a_t)
        ...
        dir_xt = (1. - a_prev - sigma_t**2).sqrt() * et
        ...
        ret_dict = {"x_noise": x_prev, "pred_x0": pred_x0, "loss": loss}
        return ret_dict