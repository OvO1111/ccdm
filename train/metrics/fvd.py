...
class Embedder:
    ...
    def __init__(self, videos=None):
        ...
        module_spec = "https://www.kaggle.com/models/deepmind/i3d-kinetics/frameworks/TensorFlow1/variations/600/versions/1"
        self.model = hub.load(module_spec)
    ...
    def create_id3_embedding(self,videos):
        ...
        tensor_name = "RGB/inception_i3d/Mean:0"
        if not _is_in_graph(tensor_name):
            # i3d_model = hub.Module(module_spec, name=module_name)
            x = self.model(videos)
        
        #   tensor = self.model.graph.get_tensor_by_name(tensor_name)

        return x
  
  
def calculate_fvd1(real_activations, generated_activations):
    return tfgan.eval.frechet_classifier_distance_from_activations(
        real_activations, generated_activations)


def calculate_fvd2(m, m_w, sigma, sigma_w):
    ...
    def _symmetric_matrix_square_root(mat, eps=1e-10):
        ...
        return np.matmul(np.matmul(u, np.diag(si)), v, transpose_b=True)
    
    def _calculate_fid(m, m_w, sigma, sigma_w):
        sqrt_sigma = _symmetric_matrix_square_root(sigma)
        sqrt_a_sigmav_a = np.matmul(sqrt_sigma, np.matmul(sigma_w, sqrt_sigma))
        sqrt_trace_component = np.trace(_symmetric_matrix_square_root(sqrt_a_sigmav_a))
        trace = np.trace(sigma + sigma_w) - 2.0 * sqrt_trace_component

        # Next the distance between means.
        mean = np.sum(m ** 2 - m_w ** 2)  # Equivalent to L2 but more stable.
        fid = trace + mean
        return fid
        
    m, m_w, sigma, sigma_w = map(lambda x: x.numpy())
    fvd = tuple(
...
def compute_fvd(embedder_real, embedder_fake, device, real_videos, fake_videos):
    ...
    with tf.device(f"/gpu:{device}"):
        with g.as_default():
            ...
            embed_real = embedder_real.create_id3_embedding(real_pre)
            fake_pre = preprocess(input_fake, (224, 224))
            # emb_fake = Embedder(fake_pre)
            embed_fake = embedder_fake.create_id3_embedding(fake_pre)
            feed_dict = {input_real: real_videos.cpu(), input_fake: fake_videos.cpu()}
    ...
    real = r["default"]
    fake = f["default"]
    embed_real = tf.placeholder(dtype=tf.float32, shape=(real.shape[0], 600))
    embed_fake = tf.placeholder(dtype=tf.float32, shape=(real.shape[0], 600))
    result = calculate_fvd1(embed_real, embed_fake)
    feed_dict = {embed_real: real, embed_fake: fake}
    fvd_val = sess.run(result, feed_dict)
    sess.close()
    if isinstance(fvd_val, tuple):
        fvd_val = calculate_fvd2(fvd_val)
