...
from train.loss import DiceLoss
from train.utils import BasicLogger
...
class ColonSegmentation(Dataset):
    def __init__(self, split="train", patch=(64, 128, 128)) -> None:
        super().__init__()
        self.files = "/mnt/data/oss_beijing/dailinrui/data/cmu/splitall/mask"
        self.ds = list(pb.Path(self.files).glob("*.nii.gz"))
        self.train_ds = self.ds[:round(.9 * len(self.ds))]
        self.val_ds = self.ds[round(.9 * len(self.ds)):]
        self.split_ds = self.train_ds if split == "train" else self.val_ds
        
        self.cache = {}
        self.resize = tio.transforms.Resize(patch)
    ...
    def __getitem__(self, index):
        # sample = self.cache.get(index, self.load_fn(self.split_ds[index]))
        sample = self.load_fn(self.split_ds[index])
        
        s, e = np.where(sample.sum((1, 2)))[0][[0, -1]]
        very_raw = sample[s: e + 1]
        segmented = conserve_only_certain_labels(sample[s: e + 1])
        unsegmented = (segmented > 0).astype(np.uint8)
        
        subject = tio.Subject(seg=tio.LabelMap(tensor=segmented[None]),
                              raw=tio.LabelMap(tensor=unsegmented[None]),
                              rawer=tio.LabelMap(tensor=very_raw[None]))
        subject = self.resize(subject)
        subject = self.transforms(subject)
        
        return {"seg": subject.seg.data, "raw": subject.raw.data, "totalseg": subject.rawer.data}
...
class Trainer:
    ...
    def __init__(self, cuda_index=0) -> None:
        ...
        self.train_dl = DataLoader(self.train_ds, self.batch_size, shuffle=True, pin_memory=True, num_workers=16)
        self.val_dl = DataLoader(self.val_ds, batch_size=1, pin_memory=True, num_workers=1)
        
        self.train_it = tqdm(range(self.max_epochs * len(self.train_dl)), desc="train progress")
        ...
        self.loss_dice = DiceLoss(n_classes=5)
    ...
    def _train(self, ep):
        for itr, batch in enumerate(self.train_dl):
            ...
            loss_ce = self.loss_ce(out, seg[:, 0].long())
            loss_dice = self.loss_dice(soft_out, seg[:, 0].long())
    ...
    def val(model, optim, ep, val_dict, best, val_dl, cuda_index, _tensorboard_path, _model_path, _save):
        ...
        logger = {"nifti": BasicLogger("nifti", 10, os.path.dirname(_tensorboard_path))}
        ...
        with torch.no_grad():
            for itr, batch in val_it:
                ...
                val_it.set_postfix(dice=str(tuple(round(met[:, i, 0].sum() / (itr + 1), 2) for i in range(5))),
                                precision=str(tuple(round(met[:, i, 1].sum() / (itr + 1), 2) for i in range(5))),
                                recall=str(tuple(round(met[:, i, 2].sum() / (itr + 1), 2) for i in range(5))))
                logger["nifti"](pred[0], f"val_gn{itr}_seg.nii.gz")
                logger["nifti"](raw[0, 0], f"val_gn{itr}_raw.nii.gz")
    ...
    def loss_ce(self, colon_mask, tumor_mask, gt):
        with torch.no_grad():
...
if __name__ == "__main__":
    trainer = Trainer()
    # trainer.train()
    trainer._load("/mnt/workspace/dailinrui/data/pretrained/ccdm/segmentation/colon_segment/model/best_model_dice=0.79.ckpt")
    trainer.val(trainer.model, trainer.optim, trainer.max_epochs, None, trainer.best, trainer.val_dl, 0, trainer._tensorboard_path, trainer._model_path, trainer._save)
