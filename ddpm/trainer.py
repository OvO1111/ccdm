...
def grid_of_predictions(model: Model, feature_cond_encoder: ConditionEncoder, loader: DataLoader, num_predictions: int, cond_vis_fn: FunctionType,
    ...
    xts_ = List[Tensor] = []
    frozen_bert_embedder = FrozenBERTEmbedder()
    ...
    for batch in tqdm(loader):
        ...
        context_b = None if text_b is None else frozen_bert_embedder(text_b)
        ...
        for _ in range(num_predictions):
            ...
            y = model(x, condition_b_enc, feature_condition_b_enc, context=context_b)["diffusion_out"]
    ...
    predictions = torch.cat(predictions_).cpu()
    conditions = torch.cat(conditions_).cpu()
    xts = torch.cat(xts_).cpu()
    n_slices = 10
    if params["dims"] == 3 and predictions.shape[1] > n_slices:
        random_slice = random.randint(0, predictions.shape[1]-n_slices-1)
        random_slice = torch.arange(random_slice, random_slice + n_slices)
        labels = labels[:, random_slice]
        predictions = predictions[:, random_slice]
        conditions = conditions[:, random_slice]
        xts = xts[:, random_slice]
    ...
    if params["dims"] == 2:
        return dict(gt=labels[:, 0], pred=predictions[:, 0], cond=conditions[:, 0, None], xt=xts[:, 0]), 8
    elif params["dims"] == 3:
        return dict(gt=labels.view((-1,)+labels.shape[2:]),
                    pred=predictions.view((-1,)+predictions.shape[2:]),
                    cond=conditions.view((-1,)+conditions.shape[2:])[:, None],
                    xt=xts.view((-1,)+xts.shape[2:]),), labels.shape[1]
...
class Trainer:
    ...
    def train_step(self, engine: Engine, batch) -> dict:
        ...
        context = None if text is None else self.frozen_bert_embedder(text)
        ...
        ret = self.model(xt.contiguous(), condition.contiguous(), feature_condition, t, context=context)
        ...
        if engine.state.iteration % self.params["train_vis_freq"] == 0 and engine.state.iteration > 0:
            # save tensorboard results
            xt = _onehot_to_color_image(xt, self.params).squeeze(0)
            x = _onehot_to_color_image(x0, self.params).squeeze(0)
            pred = _onehot_to_color_image(x0pred, self.params).squeeze(0)
            cond = image if self.params["dims"] == 2 else image.squeeze(0).permute(1, 0, 2, 3)
            ...
            self.writer.add_image("train/xt", make_grid(xt, nrow=n), engine.state.iteration)
        ...
        self.writer.add_scalar("train/diffusion_kl_loss", loss.item(), engine.state.iteration)
        # self.writer.add_scalar("train/l1_loss", l1_loss.item(), engine.state.iteration)
...
def build_engine(trainer: Trainer,
    ...
    def test(_: Engine):
        if 'lidc' in params["dataset_file"]:
            ...
            engine_test.run(train_loader, max_epochs=1)
...
def _build_datasets(params: dict) -> Tuple[DataLoader, DataLoader, torch.Tensor, int, dict]:
    ...
    get_weights = getattr(dataset_module, 'get_weights', lambda: torch.ones(train_dataset[0]["mask"].shape[0]))
...
def run_train(local_rank: int, params: dict, exp_name: str=None):
    ...
    trainer = Trainer(polyak, optimizer, lr_scheduler, class_weights.to(idist.device()), debug_checkpoint,
                      feature_cond_polyak, params, frozen_bert_embedder, writer)
