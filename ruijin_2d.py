...
class PretrainDataset(Dataset):
    ...
    def __init__(self, split="train", use_summary=False, max_size=None):
        ...
        for p in range(n_procs):
            pool.append(mp.Process(target=self._preload, 
                                   args=(self.split_keys[p:cache_len:n_procs], _data_cache)))
            pool[-1].start()
            
        for p in pool:
            p.join()
            
        self.data_cache = dict(**_data_cache)
    ...
    def _preload(self, keys, cache):
        load_fn_im = lambda n: sitk.GetArrayFromImage(sitk.ReadImage(n))[None]
        load_fn_msk = lambda n: sitk.GetArrayFromImage(sitk.ReadImage(n)).astype(np.uint8)[None]
        for m in tqdm(keys, desc="data preload", total=len(keys), ):
            ct = load_fn_im(self.data[m]["ct"])
            totalseg = load_fn_msk(self.data[m]["totalseg"])
            crcseg = load_fn_msk(self.data[m]["crcseg"])
            
            ct_shared = sharedmem.empty(np.shape(ct), ct.dtype)
            totalseg_shared = sharedmem.empty(np.shape(totalseg), totalseg.dtype)
            crcseg_shared = sharedmem.empty(np.shape(crcseg), crcseg.dtype)
            ct_shared[:] = ct
            totalseg_shared[:] = totalseg
            crcseg_shared[:] = crcseg
            cache[m] = {"ct": ct_shared, "totalseg": totalseg_shared, "crcseg": crcseg_shared}
        return cache
